...
static inline void gen_rand_mac(uchar *mac_addr)
{
    int i, j;
    ulong timer = (*(volatile ulong *)(CONFIG_SYS_TIMERBASE+4));
    for (i = 0; i < 6; i++) {
        mac_addr[i] = (uchar)(timer & 0xFF);
        j = ((timer >> 4) & 0xFF) * 700;
        for (; j > 0; j--)
            timer = (*(volatile ulong *)(CONFIG_SYS_TIMERBASE+4));
    }
...
//		else
#if defined(CONFIG_NS115_PAD_REF) || defined (CONFIG_NS115_PAD_PROTOTYPE)
#ifdef CONFIG_DDR_512M
		gd->fb_base = 0x98600000;
#else
		gd->fb_base = 0xb3800000;
#endif
#endif
#ifdef	CONFIG_NS115_HDMI_STICK
#if defined CONFIG_DDR_512M
		gd->fb_base = 0x98600000;
#elif defined CONFIG_DUAL_DISPLAY
...
//		gd->fb_base = 0xb3800000;
//		gd->fb_base = addr;
        printk("[pchen]gd->fb_base = 0x%x\n",gd->fb_base);
	}
#endif /* CONFIG_LCD */

	unsigned int mmc_cont = (*((unsigned int*)0x0704e04c)) ? 1 : 0;
	unsigned int recovery_flag = 1;
//#if defined(CONFIG_FASTBOOT_RECOVERY) && defined (CONFIG_UBOOT_CHARGER)
...
//	env_relocate ();
...
#if defined(CONFIG_NS115_HDMI_STICK)
			if(mmc_cont == 0){
				char *mmc_read[] = {"mmc", "read", "0", "0x80007fc0","8800","7400" };
				do_mmc(NULL, 0, 6, mmc_read);
			}else if(mmc_cont == 1) {
				char *mmc_read[] = {"mmc", "read", "1", "0x80007fc0","8800","7400" };
				do_mmc(NULL, 0, 6, mmc_read);
			}
#else
			if(mmc_cont == 0){
	...
	if(mmc_cont == 1){
		char *mmc_read[] = {"mmc", "read", "1", "0x80006000","4601","1" };
		do_mmc(NULL, 0, 6, mmc_read);
		char *mmc_buf = (char *) 0x80006000;
		printf("mac in mmc is:\n");
		printf("%s\n",mmc_buf);
		char mac_flag[] = "WIFIMAC0";
		char mac_buf[20];
		if(!strncmp(mmc_buf,mac_flag,strlen(mac_flag))){
			memcpy(mac_buf,mmc_buf + 28,12);
			sprintf((char *)mac_buf,"%c%c:%c%c:%c%c:%c%c:%c%c:%c%c",mmc_buf[28],mmc_buf[29],
				mmc_buf[30],mmc_buf[31],mmc_buf[32],mmc_buf[33],
				mmc_buf[34],mmc_buf[35],mmc_buf[36],mmc_buf[37],
				mmc_buf[38],mmc_buf[39]);
			setenv("wifimac",mac_buf);
		}
		printf("eth mac is:\n");
		int eth_offset = 41;
		printf("%s\n",mmc_buf + eth_offset);
		char eth_mac_flag[] = "ETH0MAC0";
		if(!strncmp(mmc_buf + eth_offset,eth_mac_flag,strlen(eth_mac_flag))){//gen an init ethernet mac
			memcpy(mac_buf,mmc_buf + eth_offset + 28,12);
			sprintf((char *)mac_buf,"%c%c:%c%c:%c%c:%c%c:%c%c:%c%c",mmc_buf[eth_offset + 28],mmc_buf[eth_offset + 29],
				mmc_buf[eth_offset + 30],mmc_buf[eth_offset + 31],mmc_buf[eth_offset + 32],mmc_buf[eth_offset + 33],
				mmc_buf[eth_offset + 34],mmc_buf[eth_offset + 35],mmc_buf[eth_offset + 36],mmc_buf[eth_offset + 37],
				mmc_buf[eth_offset + 38],mmc_buf[eth_offset + 39]);
			setenv("ethmac",mac_buf);
        }
	}
#endif
