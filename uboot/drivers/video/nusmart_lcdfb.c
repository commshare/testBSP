...
#include "../hdmi/ch7026.h"
#include "../hdmi/cs8556.h"

int lcd_line_length;
int lcd_line_length_2;
int lcd_color_fg;
int lcd_color_bg;
...
vidinfo_t panel_info_2 = {    //NULL, 60, 1024, 600, 19841, 240, 48, 12, 3, 32, 10        0, FB_VMODE_NONINTERLACED
	vl_col:		1024,
	vl_row:		600,
	vl_bpix:	5,
	vl_hsync_len:	32,
...
#if defined(CONFIG_LCD_SLAVE_800_600_60)
vidinfo_t panel_info_2 = {    //NULL, 60, 1024, 600, 19841, 240, 48, 12, 3, 32, 10        0, FB_VMODE_NONINTERLACED
	vl_col:		800,
	vl_row:		600,
	vl_bpix:	5,
...
#ifdef CONFIG_DUAL_DISPLAY	//James	DUAL_DISPLAY
void lcd2_disable(void)
{
	unsigned int value;

	value = lcdc_readl(panel_info_2.mmio,NUSMART_LCDC_CTRL);
	if((value & NUSMART_LCDC_ENABLE)){
		value = value & (~NUSMART_LCDC_ENABLE);
	}
	lcdc_writel(panel_info_2.mmio, NUSMART_LCDC_CTRL, value);
...
void bpix2_init(void)
{
	if(disp_flag == 0)
		panel_info_2.vl_bpix = 4;		//16bit
	else if(disp_flag == 1)
		panel_info_2.vl_bpix = 5;		//32bit
}

void lcd2_ctrl_init(void *lcdbase)
{
	unsigned long value,burst_len;

	debug("entering lcd2_ctrl_init\n");

#if defined(CONFIG_LCD_SLAVE_1024_600_60)
	ns2815_pxl0_clk_set_rate(50400000,1);	/* for 1024x600 */
#endif
#if defined(CONFIG_LCD_SLAVE_PAL)
	ns2815_pxl0_clk_set_rate(27000000,1);   /* for PAL */
#endif
#if defined(CONFIG_LCD_SLAVE_800_600_60)
	ns2815_pxl0_clk_set_rate(40000000,1);	/* for 1024x600 */
#endif


	bpix2_init();
	lcd_line_length_2 = (panel_info_2.vl_col * NBITS (panel_info_2.vl_bpix)) / 8;
	/* Turn off the LCD controller */
	lcdc_writel(panel_info_2.mmio, NUSMART_LCDC_CTRL,0);

	/* Vertical timing */
	value = INTV(panel_info_2.vl_upper_margin,\
			panel_info_2.vl_lower_margin,\
			panel_info_2.vl_vsync_len );
	lcdc_writel(panel_info_2.mmio, NUSMART_LCDC_VINTV, value);

	/* Horizontal timing */
	value = INTV(panel_info_2.vl_left_margin,\
			panel_info_2.vl_right_margin,\
			panel_info_2.vl_hsync_len);
	lcdc_writel(panel_info_2.mmio, NUSMART_LCDC_HINTV, value);

	/* Display size */
	value = HV_SIZE(panel_info_2.vl_col,\
			panel_info_2.vl_row);
	lcdc_writel(panel_info_2.mmio, NUSMART_LCDC_HVSIZE, value);

	lcd_base_pri = lcdbase;
	lcdc_writel(panel_info_2.mmio, NUSMART_LCDC_PRIPTR, (u_long)lcdbase);

	lcd_base_sec = (lcdbase + (panel_info_2.vl_col * panel_info_2.vl_row * NBITS(panel_info_2.vl_bpix))/8);
	lcdc_writel(panel_info_2.mmio, NUSMART_LCDC_SECPTR, (u_long)lcd_base_sec);

	value = lcdc_readl(NUSMART_LCDC_BASE,NUSMART_LCDC_AXI);
	if(panel_info_2.vl_bpix <= 4){					//set burst length
	...
	lcdc_writel(panel_info_2.mmio, NUSMART_LCDC_AXI, value);
	value =  (panel_info_2.vl_col * panel_info_2.vl_row / ((panel_info_2.vl_bpix > 4 ? 4: 8) * burst_len)) - 1;

	lcdc_writel(panel_info_2.mmio, NUSMART_LCDC_TRAN_NUM, value);

	debug("exiting lcd2_ctrl_init %d\n",panel_info_2.vl_bpix);
}


ulong calc_fb2size(void)
{
	return ((panel_info_2.vl_col * panel_info_2.vl_row *
				NBITS(panel_info_2.vl_bpix)) / 8) + PAGE_SIZE;
}
...
void lcd2_newline(void * row_sec, int scroll_size)
{
#if 1
	unsigned int value = lcdc_readl(panel_info_2.mmio, NUSMART_LCDC_CTRL);
...
#ifdef CONFIG_NUSMART_CH7026
	ch7026_probe(0x76);
#endif
#ifdef CONFIG_NUSMART_CS8556
	cs8556_probe(0x3d);
#endif
...
void lcd_newline(void * row_sec, int scroll_size)
{
	...
	lcd_enable();
#ifdef CONFIG_DUAL_DISPLAY	//James	DUAL_DISPLAY
	lcd2_newline(row_sec,scroll_size);
#endif
}
