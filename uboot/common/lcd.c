...
#if defined(CONFIG_DUAL_DISPLAY) && defined(CONFIG_NS115_HDMI_STICK)	//James
	memset ((char *)0xb3800000,
		COLOR_MASK(lcd_getbgcolor()),
		lcd_line_length*panel_info.vl_row);
#endif
#endif
...
#ifdef CONFIG_DUAL_DISPLAY
	fb2   = (uchar *)(0xb3800000 + y * lcd_line_length_2 + x);
#endif
...
#ifdef CONFIG_DUAL_DISPLAY
			memcpy (fb2, bmap, BMP_LOGO_WIDTH);
#endif
			bmap += BMP_LOGO_WIDTH;
			fb   += panel_info.vl_col;
#ifdef CONFIG_DUAL_DISPLAY
			fb2   += panel_info_2.vl_col;
#endif
		}
	}
...
//#endif /* CONFIG_SPLASH_SCREEN_ALIGN */
	...
	debug("panel_info.vl_row = %d,pwidth = %d\n",panel_info.vl_row,pwidth);
	debug("width  = %d,height = %d\n",width,height);

	debug("x = %d y= %d %d\n",x,y,__LINE__);
	debug("bmp->header.data_offset = 0x%x\n",bmp->header.data_offset);
...
#ifdef CONFIG_DUAL_DISPLAY
#if defined CONFIG_LCD_SLAVE_PAL && defined CONFIG_NUSMART_CS8556
	fb2   = (uchar *) (0xb3800000 +
		(y + 675 - 1) * lcd_line_length_2 + x * bpix / 8);
#else
	fb2   = (uchar *) (0xb3800000 +
		((panel_info.vl_row - height ) / 2 + height - 1) * lcd_line_length_2 + (panel_info.vl_col - width) / 2 * bpix / 8);
#endif
#endif
...
#ifdef CONFIG_DUAL_DISPLAY
					*(uint16_t *)fb2 = cmap_base[*(bmap++)];
					fb2 += sizeof(uint16_t) / sizeof(*fb2);
#endif
				}
			}
...
#ifdef CONFIG_DUAL_DISPLAY
			fb2   -= (byte_width + lcd_line_length_2);
#endif
		}
		break;
...
#ifdef CONFIG_DUAL_DISPLAY
			fb2  -= (lcd_line_length_2 + width * (bpix / 8));
#endif
		}
		break;
...
*/
				...
				*(fb++) = a;
...
#ifdef CONFIG_DUAL_DISPLAY
				if(width > panel_info_2.vl_col && j>= (width-panel_info_2.vl_col)/2 && j<((width-panel_info_2.vl_col)/2+panel_info_2.vl_col))
				{
					*(fb2++) = b;
					*(fb2++) = g;
					*(fb2++) = r;
				...
				else if (j< (width-panel_info_2.vl_col)/2)
				{
				}
				else
						fb2+=4*2;
