...
{
	int idx = 0;
	void * __iomem base = __io_address(TL7689_MEM_MAX0_BASE);
...
{
	void __iomem * lcdc0 = __io_address(TL7689_DISPCTR0_BASE);
	void __iomem * lcdc1 = __io_address(TL7689_DISPCTR1_BASE);
...
void print_pinmux(void) {
	...
	unsigned int val = 0;
	void __iomem * addr = __io_address(TL7689_SCM_BASE);

	for(idx = 0; idx < TL7689_PINMUX_SIZE; idx++) {
...
int pinmux_init(unsigned int * setting, int len)
{
	int idx = 0, pflag = 1;
	void __iomem * addr = __io_address(TL7689_SCM_BASE);
	if(len != TL7689_PINMUX_SIZE) {
		printk(KERN_ERR"PINMUX init data do not match TL7689.\n");
		len = len > TL7689_PINMUX_SIZE?TL7689_PINMUX_SIZE:len;
		pflag = 1;
	}
	for(idx = 0; idx < len; idx++) {
