/*
 *  linux/arch/arm/mach-tl7689/timer-dw.c
 *
...
static void __init tl7689_clockevents_init(unsigned int timer_irq)
{
	...
	printk(KERN_INFO "tl7689_clockevents_init, timer0, mult = %x\n",
			_timer0_clockevent.mult);
 ...
 */
static irqreturn_t tl7689_timer_interrupt(int irq, void *dev_id)
{
...
static struct irqaction tl7689_timer_irq = {
	.name		= "TL7689 Timer Tick",
	.flags		= IRQF_DISABLED | IRQF_TIMER | IRQF_IRQPOLL,
	.handler	= tl7689_timer_interrupt,
	.dev_id		= &_timer0_clockevent,
};

static cycle_t tl7689_get_cycles(struct clocksource *cs)
{
	return ~readl(_timer3_va_base + TIMERX_VALUE);
}

void tl7689_timer_resume(struct clocksource *cs)
{
...
void tl7689_timer_suspend(struct clocksource *cs)
{
...
static struct clocksource clocksource_tl7689 = {
	.name		= "TL7689 timer3",
	.rating		= 200,
	.read		= tl7689_get_cycles,
	.mask		= CLOCKSOURCE_MASK(32),
	.shift		= 20,
	.flags		= CLOCK_SOURCE_IS_CONTINUOUS,
	.suspend	= tl7689_timer_suspend,
	.resume		= tl7689_timer_resume,
};

static void __init tl7689_clocksource_init(void)
{
	...
	setup_sched_clock(tl7689_get_cycles, 32, TIMER_CLK_RATE);
	clocksource_register_hz(&clocksource_tl7689, TIMER_CLK_RATE);

}

void __init tl7689_timer_init(void)
{
	//select timer's clk

	_timer0_va_base = __io_address(TL7689_TIMER1_BASE);
	_timer3_va_base = _timer0_va_base + 0x14 * 3;

	writel(0, TIMERX_CTRL + _timer0_va_base);
	writel(0, TIMERX_CTRL + _timer3_va_base);

	setup_irq(IRQ_TL7689_TIMER1_INTR0, &tl7689_timer_irq);
	tl7689_clocksource_init();
	tl7689_clockevents_init(IRQ_TL7689_TIMER1_INTR0);
}
