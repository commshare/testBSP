...
#define GPIO_PWEN	(8+('c'-'a')*32+22)	// gpio_pc22
#define GPIO_AFEN	(8+('a'-'a')*32+15)	// gpio_pa15
#define CLK_ID		"ns115_alt0"
#define CLK_RATE	24000000
	...
	ret = gpio_request(GPIO_AFEN, "ov5640");
	if (ret) {
		goto err_afen;
	}
	...
	if (on) {
		...
		gpio_direction_output(GPIO_RST, 1);
		mdelay(20);
		gpio_direction_output(GPIO_AFEN, 1);
	}
	else {
		gpio_direction_output(GPIO_AFEN, 0);
		gpio_direction_output(GPIO_RST, 0);
		clk_disable(clk);
		gpio_direction_output(GPIO_PWEN, 0);
		gpio_direction_output(GPIO_PWDN, 1);
...
static struct i2c_board_info gc2035_camera_i2c = {
	I2C_BOARD_INFO("gc2035", 0x78 >> 1),
};

static int gc2035_power(struct device * dev, int on)
{
#define GPIO_PWDN	(8+('d'-'a')*32+5)	// gpio_pd5
#define GPIO_RST	(8+('d'-'a')*32+4)	// gpio_pd4
#define GPIO_PWEN	(8+('c'-'a')*32+22)	// gpio_pc22
...
#define CLK_RATE	24000000
	...
	ret = gpio_request(GPIO_PWDN, "gc2035");
	if (ret) {
		return ret;
	}

	ret = gpio_request(GPIO_RST, "gc2035");
	if (ret) {
		goto err_rst;
	}

	ret = gpio_request(GPIO_PWEN, "gc2035");
	if (ret) {
		goto err_pwen;
	}
...
static struct i2c_board_info gc0311_camera_i2c = {
	I2C_BOARD_INFO("gc0311", 0x66 >> 1),
};

static int gc0311_power(struct device * dev, int on)
{
#define GPIO_PWDN	(8+('d'-'a')*32+5)	// gpio_pd5
#define GPIO_RST	(8+('d'-'a')*32+4)	// gpio_pd4
#define GPIO_PWEN	(8+('c'-'a')*32+22)	// gpio_pc22
...
#define CLK_RATE	24000000
	...
	ret = gpio_request(GPIO_PWDN, "gc0311");
	if (ret) {
		return ret;
	}

	ret = gpio_request(GPIO_RST, "gc0311");
	if (ret) {
		goto err_rst;
	}

	ret = gpio_request(GPIO_PWEN, "gc0311");
	if (ret) {
		goto err_pwen;
	}
...
static struct i2c_board_info gc0328_camera_i2c = {
	I2C_BOARD_INFO("gc0328", 0x42 >> 1),
};

static int gc0328_power(struct device * dev, int on)
{
#define GPIO_PWDN	(8+('d'-'a')*32+5)	// gpio_pd5
#define GPIO_RST	(8+('d'-'a')*32+4)	// gpio_pd4
#define GPIO_PWEN	(8+('c'-'a')*32+22)	// gpio_pc22
...
#define CLK_RATE	24000000
	...
	ret = gpio_request(GPIO_PWDN, "gc0328");
	if (ret) {
		return ret;
	}

	ret = gpio_request(GPIO_RST, "gc0328");
	if (ret) {
		goto err_rst;
	}

	ret = gpio_request(GPIO_PWEN, "gc0328");
	if (ret) {
		goto err_pwen;
	}
...
static struct soc_plat_dev plat_devs[] =
{
	...
	SOC_PLAT_DEV(&gc2035_camera_device,	&iclink[2]),
#endif
#ifdef CONFIG_SOC_CAMERA_GC0311
	SOC_PLAT_DEV(&gc0311_camera_device,	&iclink[3]),
#endif
#ifdef CONFIG_SOC_CAMERA_GC0328
	SOC_PLAT_DEV(&gc0328_camera_device,	&iclink[4]),
#endif
#ifdef CONFIG_SOC_CAMERA_GC0308
	SOC_PLAT_DEV(&gc0308_camera_device,	&iclink[5]),
#endif
#endif
...
#endif
	SOC_PLAT_DEV(&ns115_hdmi_device,        NULL),
...
#endif
#endif
#if defined(CONFIG_NS115_BATTERY) && !defined(CONFIG_OF)
...
static int of_ns115_int_pinmux()
{
	...
	const __be32 *list;
	u32 size=0,pin_setting[NS115_PINMUX_SIZE] = {0};
	u32 i=0;
	...
	if (of_device_is_compatible(of_pinmux, pinmux_compat)) {
		...
		if(size == 0 || size > NS115_PINMUX_SIZE * sizeof(int) || list == NULL) {
...
static void __init ns115_dt_init(void)
{
	...
	early_print("%d at %s\n",__LINE__,__func__);
//	irq_domain_generate_simple(ns115_dt_gic_match, NS115_GIC_DIST_BASE, IRQ_NS115_GIC_START);
	common_init();
	of_ns115_int_pinmux();
#ifdef CONFIG_SND_SOC_ALC5631
	//rt5631_gpio_ref_init();
	of_ns115_init_gpio_mute_amp();
#endif
//	NS115_PINMUX_INIT(pad_refboard);
...
{
	pr_info("enter in %s\n",__func__);
	of_gic_init_irq(ns115_dt_gic_match);
	ns115_init_gpio(&ref_data);
}
#endif
...
MACHINE_END
#else
DT_MACHINE_START(NS115_PAD_REF, "NUFRONT-NS115-PAD-REF")
	.map_io		= ns115_map_io,
