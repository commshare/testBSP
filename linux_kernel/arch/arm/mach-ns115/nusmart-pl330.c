/* linux/arch/arm/mach-ns115/nusmart-pl330.c
 *
 ...
 * @sdaddr: Address provided via ns115_dma_devconfig.
        ...
        int pluck)
{
    ...
    if (pluck || !(ch->options & NS115_DMAF_CIRCULAR))
        del_from_queue(xfer);
        ...
        struct nusmart_pl330_xfer *xfer, int front)
{
    ...
    if (ch->options & NS115_DMAF_CIRCULAR &&
...
static inline void _finish_off(struct nusmart_pl330_xfer *xfer,
        enum ns115_dma_buffresult res, int ffree)
{
    struct nusmart_pl330_chan *ch;
    ...
    if (ffree || !(ch->options & NS115_DMAF_CIRCULAR))
        ...
        struct pl330_req *r)
{
    ...
    if (xfer) {
        ...
        if (ret == -EAGAIN) {
            ...
            dev_err(ch->dmac->pi->dev, "%s:%d!\n",
                    __func__, __LINE__);
            _finish_off(xfer, NS115_RES_ERR, 0);
        }
    }
...
int ns115_dma_ctrl(enum dma_ch id, enum ns115_chan_op op)
{
    struct nusmart_pl330_xfer *xfer;
    ...
    //printk(KERN_EMERG "ns115_dma_ctrl with id=%d, op=%d\n", id, op);
    spin_lock_irqsave(&res_lock, flags);
    ...
    switch (op) {
        ...
        case NS115_DMAOP_STOP:
            pl330op = PL330_OP_ABORT;
            break;

        case NS115_DMAOP_FLUSH:
            pl330op = PL330_OP_FLUSH;
            break;

        case NS115_DMAOP_PAUSE:
        case NS115_DMAOP_RESUME:
        case NS115_DMAOP_TIMEOUT:
        case NS115_DMAOP_STARTED:
            spin_unlock_irqrestore(&res_lock, flags);
            return 0;
    ...
    if (ch->req[idx].x) {
        ...
        spin_unlock_irqrestore(&res_lock, flags);
        _finish_off(xfer, NS115_RES_ABORT,
    ...
    if (pl330op == PL330_OP_FLUSH) {
        ...
        if (ch->req[1 - idx].x) {
            ...
            spin_unlock_irqrestore(&res_lock, flags);
            _finish_off(xfer, NS115_RES_ABORT, 1);
            spin_lock_irqsave(&res_lock, flags);
        }
...
EXPORT_SYMBOL(ns115_dma_ctrl);

int ns115_dma_enqueue(enum dma_ch id, void *token,
        dma_addr_t addr, int size)
{
    ...
    if (ch->options & NS115_DMAF_AUTOSTART)
        ns115_dma_ctrl(id, NS115_DMAOP_START);
...
enq_exit:
    ...
    //printk(KERN_EMERG "ns115_dma_enqueue return with %d\n", ret);
    return ret;
}
EXPORT_SYMBOL(ns115_dma_enqueue);

int ns115_dma_request(enum dma_ch id,
        struct ns115_dma_client *client,
        void *dev)
{
    struct nusmart_pl330_dmac *dmac;
...
EXPORT_SYMBOL(ns115_dma_request);

int ns115_dma_free(enum dma_ch id, struct ns115_dma_client *client)
{
    struct nusmart_pl330_chan *ch;
    struct nusmart_pl330_xfer *xfer;
    unsigned long flags;
...
EXPORT_SYMBOL(ns115_dma_free);

int ns115_dma_config(enum dma_ch id, int xferunit)
{
    struct nusmart_pl330_chan *ch;
    struct pl330_info *pi;
    unsigned long flags;
    ...
    //printk(KERN_EMERG "ns115_dma_config IN");
    spin_lock_irqsave(&res_lock, flags);
...
EXPORT_SYMBOL(ns115_dma_config);

/* Options that are supported by this driver */
#define NUSMART_PL330_FLAGS (NS115_DMAF_CIRCULAR | NS115_DMAF_AUTOSTART)

int ns115_dma_setflags(enum dma_ch id, unsigned int options)
{
    struct nusmart_pl330_chan *ch;
    unsigned long flags;
    int ret = 0;
...
EXPORT_SYMBOL(ns115_dma_setflags);

int ns115_dma_set_buffdone_fn(enum dma_ch id, ns115_dma_cbfn_t rtn)
{
    struct nusmart_pl330_chan *ch;
    unsigned long flags;
    int ret = 0;
...
EXPORT_SYMBOL(ns115_dma_set_buffdone_fn);

int ns115_dma_devconfig(enum dma_ch id, enum ns115_dmasrc source,
        unsigned long address)
{
    ...
    switch (source) {
        case NS115_DMASRC_HW: /* P->M */
            ch->req[0].rqtype = DEVTOMEM;
            ch->req[1].rqtype = DEVTOMEM;
            ch->rqcfg.src_inc = 0;
            ch->rqcfg.dst_inc = 1;
            break;
        case NS115_DMASRC_MEM: /* M->P */
...
EXPORT_SYMBOL(ns115_dma_devconfig);

int ns115_dma_getposition(enum dma_ch id, dma_addr_t *src, dma_addr_t *dst)
{
    struct nusmart_pl330_chan *ch = id_to_chan(id);
    struct pl330_chanstatus status;
    int ret;

    //printk(KERN_EMERG "ns115_dma_getposition IN\n");
    if (!ch || chan_free(ch))
        return -EINVAL;
    ...
    *dst = status.dst_addr;
    ...
    //printk(KERN_EMERG "ns115_dma_getposition src = 0x%x, dst = 0x%x\n", status.src_addr, status.dst_addr);

    return 0;
}
EXPORT_SYMBOL(ns115_dma_getposition);
...
static int pl330_remove(struct platform_device *pdev)
{
    ...
    list_for_each_entry(ch, &chan_list, node) {
        ...
        if (del) {
            spin_unlock_irqrestore(&res_lock, flags);
            ns115_dma_free(ch->id, ch->client);
            spin_lock_irqsave(&res_lock, flags);
            list_del(&ch->node);
            kfree(ch);
        }
