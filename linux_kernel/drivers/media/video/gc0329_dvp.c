...
static int set_brightness(struct i2c_client *client, int val)
{
	...
	switch (val) {
	case -4:
		ret = gc0329_write(client, 0xd5, 0xc0);
		break;
	case -3:
		ret = gc0329_write(client, 0xd5, 0xd0);
		break;
	case -2:
		ret = gc0329_write(client, 0xd5, 0xe0);
		break;
	case -1:
		ret = gc0329_write(client, 0xd5, 0xf0);
		break;
	case 0:
		ret = gc0329_write(client, 0xd5, 0x00);
		break;
	case 1:
		ret = gc0329_write(client, 0xd5, 0x10);
		break;
	case 2:
		ret = gc0329_write(client, 0xd5, 0x20);
		break;
	case 3:
		ret = gc0329_write(client, 0xd5, 0x30);
		break;
	case 4:
		ret = gc0329_write(client, 0xd5, 0x40);
		break;
	default:
		ret = -1;
	}
...
static int gc0329_s_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct gc0329_priv *priv = to_gc0329(sd);
...
static int gc0329_try_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
	gc0329_res_roundup(&mf->width, &mf->height);
