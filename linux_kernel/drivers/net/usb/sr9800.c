...
void usb_ethnet_gpio_init(void)
{
    int ret = 0;
    ret = gpio_request(8+22, "usblan");
    if(ret < 0)
        printk(KERN_ERR "usblan  gpio request failed \n");
    gpio_direction_output(8+22, 1);
    gpio_free(8+22);
}
...
static int sr9800_bind(struct usbnet *dev, struct usb_interface *intf)
{
	 ...
	 */
	if (!usbnet_getcmdline_mac(dev->net->dev_addr) && memcmp( dev->net->dev_addr, defaultAddress, ETH_ALEN ) == 0)
	{
...
//	netif_carrier_off(dev->net);
	...
	if (dev->udev->speed == USB_SPEED_HIGH) {
		if ((ret = sr_write_cmd (dev, 0x2A,
				SR9800_BULKIN_SIZE[0].byte_cnt,
				SR9800_BULKIN_SIZE[0].threshold,
				0, NULL)) < 0) {
			netdev_err(dev->net, "Reset RX_CTL failed: %d", ret);
			goto out;
		}
		dev->rx_urb_size = SR9800_BULKIN_SIZE[0].size;
	} else {
		if ((ret = sr_write_cmd (dev, 0x2A,
