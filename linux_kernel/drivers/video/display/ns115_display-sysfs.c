...
{
	struct ns115_display_device *dsp = dev_get_drvdata(dev);
...
void ns115_display_device_enable(struct ns115_display_device *ddev)
{
	struct list_head *pos, *head;
	struct ns115_display_device *dev = NULL, *dev_enabled = NULL, *dev_enable = NULL;
	int enable = 0,connect, has_connect = 0;
	
	head = &display_device_list;		
	list_for_each(pos, head) {
		dev = list_entry(pos, struct ns115_display_device, list);
...
EXPORT_SYMBOL(ns115_display_device_enable);

void ns115_display_device_select(int property, int priority)
{
	struct list_head *pos, *head;
...
EXPORT_SYMBOL(ns115_display_device_select);

static struct mutex allocated_dsp_lock;
static DEFINE_IDR(allocated_dsp);
static struct class *display_class;

struct ns115_display_device *ns115_display_device_register(struct ns115_display_driver *driver,
						struct device *parent, void *devdata)
{
	struct ns115_display_device *new_dev = NULL;
	int ret = -EINVAL;
	...
	new_dev = kzalloc(sizeof(struct ns115_display_device), GFP_KERNEL);
...
EXPORT_SYMBOL(ns115_display_device_register);

void ns115_display_device_unregister(struct ns115_display_device *ddev)
{
	if (!ddev)
		return;
...
EXPORT_SYMBOL(ns115_display_device_unregister);

static int __init ns115_display_class_init(void)
{
...
static void __exit ns115_display_class_exit(void)
{
	class_destroy(display_class);
}

subsys_initcall(ns115_display_class_init);
module_exit(ns115_display_class_exit);


MODULE_AUTHOR("ping.chen@nufront.com");
MODULE_DESCRIPTION("Driver for ns115 display device");
MODULE_LICENSE("GPL");
