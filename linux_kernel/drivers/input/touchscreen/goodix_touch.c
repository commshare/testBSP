...
static int goodix_init_panel(struct goodix_ts_data *ts)
{
	...
	uint8_t  freq[4] = {194,100};
#ifdef DRIVER_SEND_CFG				//for kedi 9.7(puts your config info here,if need send config info)
	uint8_t config_info[] = {
		0x65,0x02,0x04,0x00,0x02,0x58,0x0A,0x6D,0x01,0x00,0x0F,0x26,0x02,0x08,0x10,0x00,
		0x00,0x20,0x00,0x00,0x10,0x10,0x10,0x11,0x37,0x00,0x00,0x00,0x01,0x02,0x03,0x04,
		0x05,0x06,0x07,0x08,0x09,0x0A,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0x01,0x02,0x03,0x04,
		0x05,0x06,0x07,0x08,0x09,0x0A,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x3C,0x50,0x00,
    ...
    ret=i2c_read_bytes(ts->client, freq, 2);
    printk("goodix default scan interval freq is %d\n", freq[1]);
    freq[0] = 194;
    freq[1] = 0x00;
    ret=i2c_write_bytes(ts->client,freq, 2);
    freq[0] = 194;
    freq[1] = 0x55;
    ret=i2c_read_bytes(ts->client, freq, 2);
    printk("set goodix scan interval freq as %d\n", freq[1]);
...
static void goodix_ts_work_func(struct work_struct *work)
{
	...
	static bool press_screen = false;

	static unsigned int input_x_pri[MAX_FINGER_NUM] = {0};
	static unsigned int input_y_pri[MAX_FINGER_NUM] = {0};
	static unsigned int input_w_pri[MAX_FINGER_NUM] = {0};
...
COORDINATE_POLL:
	...
	if((ts->int_trigger_type> 1)&& (gpio_get_value(ts->gpio) != (ts->int_trigger_type&0x01)))
	{
		goto NO_ACTION;
	}
#endif
	...
	if(touch_num)
	{
		...
		for(index=0; index<touch_num; index++){
			...
			if(	(input_x_pri[index] != input_x) ||
					(input_y_pri[index] != input_y) ||
					(input_w_pri[index] != input_w) ||
					(touch_num != point_num) ||
					(!press_screen))
			{
				send_data = true;
...
//	tP_platform_data = (struct touch_panel_platform_data*)ts->client->dev.platform_data;  /modif by zhanghui
//	reset_gpio = irq_to_gpio(tP_platform_data->irq_reset);
			...
			struct device_attribute *attr, char *buf)
{
	...
	diff_data_len = 2241;
	diff_data = (unsigned char*)vmalloc(diff_data_len);
	memset(diff_data, 0, diff_data_len);
	disable_irq(TS_INT);
	ts = i2c_get_clientdata(i2c_connect_client);
	...
	ret = i2c_read_bytes(ts->client, diff_data, diff_data_len);
	if(ret != 2)
	{
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct goodix_ts_data *ts;
	struct touch_panel_platform_data *tP_platform_data = NULL;
	char *version_info = NULL;
	char test_data = 1;
	...
	if(np == NULL)
	{
		printk(KERN_ERR"[get np adrr error]np= 0x%x\n",np);
		kfree(np);
		kfree(ts);
		return -1;
	}
	...
	if(ip)
	{
		...
		ts->reset_gpio = irq_to_gpio(reset_gpio);
	}
	else
	{
		printk(KERN_ERR"[get ip addr error] ip= 0x%x\n",ip);
		kfree(np);
		kfree(ip);
		kfree(ts);
		return -1;
	...
	ts->reset_gpio = irq_to_gpio(tP_platform_data->irq_reset);
#endif
	printk(KERN_EMERG "touch panel reset gpio number is %d\n",ts->reset_gpio);
	ret = gpio_request(ts->reset_gpio, "TP_RESET");
	if(ret < 0){
	...
	if (client->irq)
	{
	     ts->gpio = irq_to_gpio(client->irq);
		//ret = gpio_request(INT_PORT, "TS_INT");
		ret = gpio_request(ts->gpio, "TS_INT");
		if (ret < 0)
		{
			//dev_err(&client->dev, "Failed to request GPIO:%d, ERRNO:%d\n",(int)INT_PORT,ret);
			dev_err(&client->dev, "Failed to request GPIO:%d, ERRNO:%d\n", ts->gpio, ret);
		...
		dev_err(&client->dev, "request GPIO:%d/n", ts->gpio);
...
static int goodix_update_read( char *page, char **start, off_t off, int count, int *eof, void *data )
{
	...
	read_data = (unsigned char*)vmalloc(1201);
	read_data[0] = 80;
